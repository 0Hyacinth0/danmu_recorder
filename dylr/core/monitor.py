...
dt_host = '47.100.185.26'
dt_port = 3306
dt_user = 'byb_yxbnet_cn'
dt_passwd = 'byb_yxbnet_cn'
dt_db = 'byb_yxbnet_cn'
...
def check_room(room):
    try:
        ...
        logger.error_and_print(f"Error checking room {room.room_id}: {str(e)}")
...
def check_room_using_api(room):
    ...
    # else:
        ...
        # con = pymysql.connect(host=dt_host, port=dt_port, user=dt_user, passwd=dt_passwd, db=dt_db,
        #                       charset='utf8mb4')
        # cur = con.cursor()
        # update_sql = f"""update  fa_live set  istrue=0 where roomid='{room.room_id}'"""
        # print(time.strftime('%Y-%m-%d %H:%M:%S'), update_sql)
